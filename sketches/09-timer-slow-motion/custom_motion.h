...
typedef struct {
  int phase_count;    // now many phases in the motion 
  int phase_delay;    // delay between phases in ms
  int is_async;       //true is motion is executed from an interrupt handler
} motion_type_t;
...
void mgSetServoTarget(int index, int angle) {  
  if (index < MOTION_GROUP.frame_size) {
    ...
    MOTION_GROUP.angle_increments[index] = (MOTION_GROUP.target_frame[index]-MOTION_GROUP.current_frame[index]) / MOTION_GROUP.motion_type.phase_count;    
  }
}
...
bool mgDoMotion(void) {
    ...
    if (in_progress) {
      ...
      if (MOTION_GROUP.motion_type.is_async) {
        /*
      ...
      */
      ...
      if (!MOTION_GROUP.motion_type.is_async) {
        /*
         * for sync calls , enforce the motion delay
         */
        delay(MOTION_GROUP.motion_type.phase_delay);
      }
    }
