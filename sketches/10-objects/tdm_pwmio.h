...
namespace tdm {
  ...
  class PwmIO {
    ...
    public:
      ...
      void setup(void) {
        if (_pwm0==NULL) {
          _pwm0 = Adeept_PWMPCA9685(0x40);          
          _pwm0.begin();
          _pwm0.setPWMFreq(TDM_PWM_SERVO_FREQUENCY);
        }

        if (_pwm1==NULL) {
          _pwm1 = Adeept_PWMPCA9685(0x41);
          _pwm1.begin();
      ...
      void pwmSetPulse(int channel, int pulse_length){
        if (channel<TDM_PWM_CHANNELS_PER_CONTROLLER) {
          ...
          pwm0.setPWM(channel, TDM_PWM_PIN_OFF, pulse_length);  
        } else {
          // use second controller
          pwm1.setPWM(channel-TDM_PWM_CHANNELS_PER_CONTROLLER, TDM_PWM_PIN_OFF, pulse_length); 
        }
      }
